...
//    pStart_vec_y(TVectorD(19)), theta_true_x(TVectorD(19)), theta_true_y(TVectorD(19))
{
  ...
  TOF0refname="TOF0data.txt";
  TOF1refname="TOF1data.txt";
  TOF2refname="TOF2data.txt";
...
// retrieve_mode = 0 for absorber present and 1 for no absorber present.
  Collection USretrieved, DSretrieved, EndofDS, StartofDS, USPostFid, DSPostFid, USPostMom, DSPostMom, DSsetUncorrected, USPostExtTKU0, DSPostExtTKU0, USPostTOF01Ex, DSPostTOF01Ex,USPostTOF01,DSPostTOF01;
  Collection Collect_USTOF0, Collect_USTOF1,Collect_TOF;
  if (retrieve_mode==0){
     ...
     std::cerr<<"Entries in Collect_TOF: "<< Collect_TOF.N()<<std::endl; 
...
//---------------------------------------------------------------------------------------
    if(retrieve_mode==0){
       tof10->Fill(Collect_TOF.E(i).TOF01);
       tof21->Fill(Collect_TOF.E(i).TOF12);
       pass_TOF01->Fill(Collect_TOF.E(i).TOF01);
    }else{
       mcpass_TOF01->Fill(Collect_TOF.E(i).TOF01);
    }
    if ( Collect_TOF.E(i).TOF01 < TOF_lower_limit ||
         Collect_TOF.E(i).TOF01 > TOF_upper_limit){
//      std::cerr<<"**Failed TOF01 Cut"<<std::endl;
      if (cut_TOF01) continue;
    }
    HPt_TKUrad->Fill(Ptot);
...
//  chain->SetBranchAddress("MCEvent", &mcevent);
  ...
  Collection Collect_EndOfDS, Collect_StartOfDS, Collect_USTOF0, Collect_USTOF1;
...
//  std::cerr<<"Nentries set"<<std::endl;
  Collection USAllTOF, DSAllTOF, USPostTOF0, DSPostTOF0, USPostTKU, DSPostTKU, USPostTKUrad, DSPostTKUrad;
  Collection USPostDiff, DSPostDiff, USPostChi2, DSPostChi2, USPostFid, DSPostFid;
  Collection USPostTOF01, DSPostTOF01, USPostTOF01Ex, DSPostTOF01Ex, StartOfDS;
  Collection Collect_EndOfDS, Collect_StartOfDS, Collect_USTOF0, Collect_USTOF1;
//  double pz = 0.;
  std::cerr <<"Number of entries in data file: "<< Nentries << std::endl;
  // Loop over all tree entries.
