...
int main(int argc, char* argv[]) {
    ...
    for (int ifile = 2; ifile < argc; ifile++){
      ...
      if(!Spill) {
        std::cout<<"No Spill Data"<<std::endl;
        continue;
      }
      file->Close();
    ...
    std::cout<<"File List complete"<<std::endl;
    ...
    TH1D hrate("hrate", 
	       ...
	       240,0.5,24.5);
    TH1D histPtotal ("Ptotal","Total Momentum;Number",30,100,250);
    TH2D hr("hr", 
...
//    fname += type.substr(found-9,9);
    fname += type;
    fname += ".root";
    std::cout<<"fname set: "<<fname<<std::endl;
    ...
    mctree->Branch("GlobalBranch","MAUS::GlobalEvent",&globalevent, 64000, 10);
...
//          std::cout<<"physics event detected"<<std::endl;
...
//                std::cout<<"plane0digit tof0"<<std::endl;
		plane0digit = true;
	      } else {
//                std::cout<<"plane1digit tof0"<<std::endl;
		plane1digit = true;
	      }
	    }
...
//                std::cout<<"plane0digit tof1"<<std::endl;
		plane0digit = true;
	      } else {
//                std::cout<<"plane1digit tof1"<<std::endl;
		plane1digit = true;
	      }
	    }
	    if( plane0digit && plane1digit ){
//              std::cout<<"Write data to tree/mctree"<<std::endl;
	      TOF1count++;
	      tofevent   = tof_event;
	      ...
	      if(type1.find("data")!=std::string::npos){
                 tree->Fill();
                 if (scifievent != NULL){
//                    std::cout<<"Sci Fi event exists"<<std::endl;
...
//                       std::cout<<"Sci Fi Track"<<std::endl;
...
//                          std::cout<<"SciFi Point"<<std::endl;
                          int selected_station_tracker0 = 5;
                          int selected_plane_tracker0 = 2;
                          MAUS::SciFiTrackPoint* point = (*track_point_iter);
                          int tracker = point->tracker();
                          int station = point->station();                  
                          int plane = point->plane();
                          if (tracker == 0 && station == selected_station_tracker0 && plane == selected_plane_tracker0){ 
                             MAUS::ThreeVector position_tracker0 = point->pos();
                             MAUS::ThreeVector momentum_tracker0 = point->mom();
                             double Ptot = sqrt(momentum_tracker0.x()*momentum_tracker0.x()+momentum_tracker0.y()*momentum_tracker0.y()+momentum_tracker0.z()*momentum_tracker0.z());
//                             std::cout<<"Ptot is :"<<Ptot<<std::endl;
                             histPtotal.Fill(Ptot);
                          }
...
//                std::cout<<"plane0digit tof2"<<std::endl;
		tof2_digits_0_hist.Fill(tof2_digit.GetSlab());
		plane0digit = true;
	      } else {
//                std::cout<<"plane1digit tof2"<<std::endl;
...
//            std::cout<<"mc section"<<std::endl;
    ...
    outfile->cd();
    if(type.find("data")!=std::string::npos){
      std::cout<<"Write data to tree"<<std::endl;
      histPtotal.Write();
      TCanvas c2("beam_momentum","beam_momentum");
      c2.Draw();
      c2.SetLogz();
      hp.Draw("colz");
      c2.Print("beam_momentum.pdf");

      tree->Write();
      tree->Print();
    }
    if(type.find("mc")!=std::string::npos){
      std::cout<<"Write mc to mctree"<<std::endl;
      hrate.Write();
      hr.Write();
      hp.Write();
      hdp.Write();
